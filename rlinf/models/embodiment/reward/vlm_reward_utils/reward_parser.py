...
def extract_progress_value(text: str) -> float | None:
...
@register_reward_parser("procvlm_progress_reward_parser")
class ProcVLMProgressRewardParser(BaseRewardParser):
    def __init__(
        self,
        invalid_progress: float = 0.0,
        normalize: bool = True,
        debug_print: bool = False,
        debug_print_every: int = 10,
        debug_sample_texts: int = 2,
    ) -> None:
        self.invalid_progress = float(invalid_progress)
    ...
    def parse_rewards(self, outputs: list[str]) -> torch.Tensor:
        ...
        invalid_count = 0
        for output in outputs:
            progress = extract_progress_value(output)
            if progress is None:
                progress = self.invalid_progress
                invalid_count += 1
                if len(invalid_examples) < self.debug_sample_texts:
                    invalid_examples.append(str(output).replace("\n", "\\n")[:220])
            if self.normalize:
        ...
        if self.debug_print and (
            ...
            logger.info(
                "[RMDBG_PARSE] parser=procvlm_progress call=%d batch=%d invalid=%d mean=%.4f",
                self._call_idx,
                len(outputs),
                invalid_count,
