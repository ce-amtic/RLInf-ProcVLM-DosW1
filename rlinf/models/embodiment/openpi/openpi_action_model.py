...
class OpenPi0ForRLActionPrediction(PI0Pytorch, BasePolicy):
    ...
    def __init__(
        ...
        self._trace_nonfinite = _env_flag("OPENPI_TRACE_NONFINITE")
        self._trace_all_stats = _env_flag("OPENPI_TRACE_ALL_STATS")
        self._raise_nonfinite = _env_flag("OPENPI_RAISE_ON_NONFINITE")
        self._trace_warning_count = 0
        self._trace_warning_limit = int(os.getenv("OPENPI_TRACE_WARNING_LIMIT", "120"))
        self._trace_all_limit = int(os.getenv("OPENPI_TRACE_ALL_LIMIT", "2"))
        self._predict_trace_calls = 0

    def set_global_step(self, global_step):
        self.global_step = global_step

    @staticmethod
    def _format_tensor_range(tensor: torch.Tensor, finite_mask: torch.Tensor) -> str:
        if tensor.numel() == 0:
            return "empty"
    ...
    def _trace_tensor(self, name: str, value: Any, *, force: bool = False) -> None:
        if not (self._trace_nonfinite or force):
            return
        ...
        if should_log and self._trace_warning_count < self._trace_warning_limit:
            self._trace_warning_count += 1
            self.logger.warning(
                "OpenPI tensor trace %s: shape=%s dtype=%s device=%s "
                "invalid=%s/%s range=%s.",
                name,
                tuple(tensor.shape),
                tensor.dtype,
                ...
                invalid_count,
                tensor.numel(),
                self._format_tensor_range(tensor, finite_mask),
            )
        if invalid_count > 0 and self._raise_nonfinite:
            raise FloatingPointError(
                f"OpenPI first non-finite tensor at {name}: "
                f"invalid={invalid_count}/{tensor.numel()}, "
    ...
    def _trace_nested(
        ...
        *,
        ...
        if torch.is_tensor(value):
            self._trace_tensor(prefix, value, force=force)
        elif isinstance(value, dict):
            for key, nested_value in value.items():
                self._trace_nested(
                    f"{prefix}.{key}", nested_value, force=force, depth=depth + 1
                )
        elif isinstance(value, (list, tuple)):
            for idx, nested_value in enumerate(value):
                self._trace_nested(
                    f"{prefix}[{idx}]", nested_value, force=force, depth=depth + 1
                )

    def _warn_nonfinite(self, name: str, tensor: torch.Tensor) -> None:
        if torch.isfinite(tensor).all():
            return
        self._nonfinite_warning_count += 1
        if self._nonfinite_warning_count <= 20 or self._nonfinite_warning_count % 100 == 0:
            invalid_count = int((~torch.isfinite(tensor)).sum().item())
            self.logger.warning(
                "OpenPI sanitized non-finite tensor "
                f"{name}: count={invalid_count}/{tensor.numel()}, "
                f"warnings={self._nonfinite_warning_count}."
            )

    def _sanitize_sample_tensor(
        self,
        tensor: torch.Tensor,
        name: str,
        *,
        ...
        self._warn_nonfinite(name, tensor)
        tensor = torch.nan_to_num(tensor, nan=nan, posinf=posinf, neginf=neginf)
        if clamp is not None:
            tensor = torch.clamp(tensor, min=clamp[0], max=clamp[1])
        return tensor

    def _valid_action_mask_from_tensor(
        self, tensor: torch.Tensor | None, batch_size: int
    ) -> torch.Tensor:
        if tensor is None or not torch.is_tensor(tensor) or not tensor.dtype.is_floating_point:
            device = next(self.parameters()).device
            return torch.ones(
                batch_size, self.config.action_chunk, dtype=torch.bool, device=device
            )
        finite_mask = torch.isfinite(tensor)
        if finite_mask.shape[0] != batch_size:
            finite_mask = finite_mask.reshape(batch_size, -1)
        if finite_mask.dim() >= 3 and finite_mask.shape[1] >= self.config.action_chunk:
            return finite_mask[:, : self.config.action_chunk].reshape(
                batch_size, self.config.action_chunk, -1
            ).all(dim=-1)
        if finite_mask.dim() == 2 and finite_mask.shape[1] == self.config.action_chunk:
            return finite_mask
        return finite_mask.reshape(batch_size, -1).all(dim=-1, keepdim=True).expand(
            -1, self.config.action_chunk
        )
    ...
    def output_transform(self, outputs):
        self._trace_nested("output_transform.input", outputs)
        # split & transform
        ...
        self._trace_nested("output_transform.output", outputs)
        return outputs
    ...
    def default_forward(
        ...
        **kwargs,
        ...
        log_probs = self._sanitize_sample_tensor(
            log_probs,
            "default_logprobs",
            posinf=100.0,
            neginf=-100.0,
        ...
        value_t = self._sanitize_sample_tensor(
            value_t,
            "default_values",
            posinf=1e4,
            neginf=-1e4,
        ...
        entropy = self._sanitize_sample_tensor(entropy, "default_entropy")
        return {
            "logprobs": log_probs,
            "values": value_t,
            "entropy": entropy,
    ...
    def predict_action_batch(
        ...
        **kwargs,
        ...
        self._predict_trace_calls += 1
        trace_all = (
            self._trace_all_stats
            and self._predict_trace_calls <= self._trace_all_limit
        )
        self._trace_nested("predict.env_obs", env_obs, force=trace_all)
        to_process_obs = self.obs_processor(env_obs)  # env obs -> policy input obs
        self._trace_nested("predict.policy_obs", to_process_obs, force=trace_all)
        processed_obs = self.input_transform(
            to_process_obs, transpose=False
        )  # policy input obs -> model input obs
        self._trace_nested("predict.processed_obs_cpu", processed_obs, force=trace_all)
        processed_obs = self.precision_processor(
            processed_obs
        )  # obs precision processor
        self._trace_nested("predict.processed_obs_device", processed_obs, force=trace_all)
        observation = _model.Observation.from_dict(processed_obs)
        ...
        for tensor in (
            ...
            outputs.get("actions"),
            outputs.get("chains"),
            outputs.get("prev_logprobs"),
            outputs.get("prev_values"),
        ):
            valid_action_mask = valid_action_mask & self._valid_action_mask_from_tensor(
                tensor, actions.shape[0]
            ).to(valid_action_mask.device)

        for key in ("actions", "chains", "prev_logprobs", "prev_values"):
            if key in outputs and torch.is_tensor(outputs[key]):
                outputs[key] = self._sanitize_sample_tensor(outputs[key], key)
        if torch.is_tensor(actions):
            actions = self._sanitize_sample_tensor(actions, "env_actions")
        if torch.is_tensor(prev_logprobs):
            prev_logprobs = self._sanitize_sample_tensor(
                prev_logprobs,
                "prev_logprobs",
                posinf=100.0,
                neginf=-100.0,
        ...
        if torch.is_tensor(prev_values):
            prev_values = self._sanitize_sample_tensor(
                prev_values,
                "prev_values",
                posinf=1e4,
                neginf=-1e4,
    ...
    def sample_actions(
        ...
        self._trace_tensor("sample.noise", noise)
        self._trace_tensor("sample.observation_state", observation.state)

        images, img_masks, lang_tokens, lang_masks, state = (
            self._preprocess_observation(observation, train=False)
        )
        self._trace_nested("sample.images", images)
        self._trace_nested("sample.img_masks", img_masks)
        self._trace_tensor("sample.lang_tokens", lang_tokens)
        self._trace_tensor("sample.lang_masks", lang_masks)
        self._trace_tensor("sample.state", state)

        prefix_output, prefix_pad_masks, past_key_values = self._build_prefix_cache(
            images, img_masks, lang_tokens, lang_masks
        )
        self._trace_tensor("sample.prefix_output", prefix_output)
        self._trace_tensor("sample.prefix_pad_masks", prefix_pad_masks)
        ...
        for idx in range(num_steps):
            ...
            self._trace_tensor(f"sample.raw_x_t_mean[{idx}]", x_t_mean)
            self._trace_tensor(f"sample.raw_x_t_std[{idx}]", x_t_std)
            self._trace_tensor(f"sample.raw_value_t[{idx}]", value_t)
            self._trace_tensor(f"sample.raw_v_t[{idx}]", v_t)
            x_t_mean = self._sanitize_sample_tensor(x_t_mean, f"x_t_mean[{idx}]")
            x_t_std = self._sanitize_sample_tensor(
                x_t_std, f"x_t_std[{idx}]", clamp=(0.0, 1e3)
            )
            value_t = self._sanitize_sample_tensor(
                value_t, f"value_t[{idx}]", posinf=1e4, neginf=-1e4, clamp=(-1e4, 1e4)
            )
            v_t = self._sanitize_sample_tensor(v_t, f"v_t[{idx}]")
            # Euler step - use new tensor assignment instead of in-place operation
            x_t = x_t_mean + self.sample_noise(x_t.shape, device) * x_t_std
            x_t = self._sanitize_sample_tensor(x_t, f"x_t[{idx}]")
            self._update_nft_state(nft_state, idx, x_t_prev, v_t, x_t, sample_method)
            log_prob = self.get_logprob_norm(x_t, x_t_mean, x_t_std)
            log_prob = self._sanitize_sample_tensor(
                log_prob,
                f"log_prob[{idx}]",
                posinf=100.0,
                neginf=-100.0,
    ...
    def sample_mean_var_val(
        ...
        self._trace_tensor("sample_mean_var.state", state)
        self._trace_tensor("sample_mean_var.x_t", x_t)
        self._trace_tensor("sample_mean_var.t_input", t_input)
        self._trace_tensor("sample_mean_var.delta", delta)
        self._trace_tensor("sample_mean_var.v_t", v_t)
        self._trace_tensor("sample_mean_var.suffix_out", suffix_out)
        # value prediction
        if (
        ...
        self._trace_tensor("sample_mean_var.x0_pred", x0_pred)
        self._trace_tensor("sample_mean_var.x1_pred", x1_pred)
        ...
        self._trace_tensor("sample_mean_var.x0_weight", x0_weight)
        self._trace_tensor("sample_mean_var.x1_weight", x1_weight)
        self._trace_tensor("sample_mean_var.x_t_std", x_t_std)
        self._trace_tensor("sample_mean_var.x_t_mean", x_t_mean)
        self._trace_tensor("sample_mean_var.value_t", value_t)
    ...
    def get_suffix_out(
        ...
        self._trace_tensor("suffix.suffix_embs", suffix_embs)
        self._trace_tensor("suffix.suffix_pad_masks", suffix_pad_masks)
        self._trace_tensor("suffix.suffix_att_masks", suffix_att_masks)
        self._trace_tensor("suffix.adarms_cond", adarms_cond)
        ...
        self._trace_tensor("suffix.full_att_2d_masks", full_att_2d_masks)
        self._trace_tensor("suffix.position_ids", position_ids)

        # Prepare attention masks
        full_att_2d_masks_4d = self._prepare_attention_masks_4d(full_att_2d_masks)
        self._trace_tensor("suffix.full_att_2d_masks_4d", full_att_2d_masks_4d)
        ...
        self._trace_tensor("suffix.suffix_out", suffix_out)
        return suffix_out
    ...
    def get_velocity(self, state, x_t, timestep, prefix_pad_masks, past_key_values):
        ...
        self._trace_tensor("velocity.v_t", v_t)
    ...
    def _build_prefix_cache(self, images, img_masks, lang_tokens, lang_masks):
        ...
        self._trace_tensor("prefix.prefix_embs", prefix_embs)
        self._trace_tensor("prefix.prefix_pad_masks", prefix_pad_masks)
        self._trace_tensor("prefix.prefix_att_masks", prefix_att_masks)
        prefix_att_2d_masks = make_att_2d_masks(prefix_pad_masks, prefix_att_masks)
        prefix_position_ids = torch.cumsum(prefix_pad_masks, dim=1) - 1
        prefix_att_2d_masks_4d = self._prepare_attention_masks_4d(prefix_att_2d_masks)
        self._trace_tensor("prefix.prefix_att_2d_masks", prefix_att_2d_masks)
        self._trace_tensor("prefix.prefix_position_ids", prefix_position_ids)
        self._trace_tensor("prefix.prefix_att_2d_masks_4d", prefix_att_2d_masks_4d)
        ...
        self._trace_tensor("prefix.prefix_output", prefix_output)
    ...
    def get_logprob_norm(self, sample, mu, sigma):
        ...
        if self.config.safe_get_logprob:
            ...
            mask = sigma <= 0
            sigma_safe = torch.where(mask, torch.ones_like(sigma), sigma.abs())
            sigma_safe = torch.clamp(sigma_safe, min=1e-6, max=1e3)
