...
class EmbodiedRewardWorker(Worker):
    ...
    def __init__(self, cfg: DictConfig):
        ...
        self.reward_merge_max_requests = max(
            1, int(async_reward_cfg.get("merge_max_requests", 1))
        )
        self.reward_merge_wait_timeout_s = max(
            0.0, float(async_reward_cfg.get("merge_wait_timeout_s", 0.0))
        )
        self.reward_merge_wait_poll_s = max(
            0.0005, float(async_reward_cfg.get("merge_wait_poll_s", 0.001))
        )
        profiling_cfg = self.cfg.runner.get("profiling", {})
        self.reward_profile_enabled = bool(
            profiling_cfg.get(
                "rollout_waits",
        ...
        self.reward_profile_output_dir = str(
            profiling_cfg.get(
                "output_dir",
        ...
        self._paused = False
        self._pause_offloaded = False

    def _profile_write_jsonl(self, record: dict[str, Any]) -> None:
        if not self.reward_profile_enabled or not self.reward_profile_output_dir:
            return
        try:
            output_dir = Path(self.reward_profile_output_dir)
            output_dir.mkdir(parents=True, exist_ok=True)
            path = output_dir / f"reward_rank_{self._rank}.jsonl"
            with path.open("a", encoding="utf-8") as f:
                f.write(json.dumps(record, sort_keys=True) + "\n")
        except Exception as exc:
            self.log_warning(f"Failed to write reward profile jsonl: {exc}")
    ...
    def model_provider_func(self):
        ...
        with open_dict(model_cfg):
            ...
            model_cfg.reward_rank = self._rank
            model_cfg.global_step = getattr(self, "global_step", 0)
        model = reward_cls(model_cfg)
        if hasattr(model, "set_worker_info"):
            model.set_worker_info(self._rank)

        return model

    def set_global_step(self, global_step: int) -> None:
        self.global_step = int(global_step)
        if hasattr(self, "model") and hasattr(self.model, "set_global_step"):
    ...
    def init_worker(self):
        ...
        self.model.eval()
        if hasattr(self.model, "eager_initialize"):
            self.model.eager_initialize()

    async def pause_async_compute(
        self, offload_model: bool = True, timeout_s: float = 2.0
    ) -> bool:
        """Best-effort pause for long-running async reward inference."""
        self._pause_requested = True
        if self._interact_task is None or self._interact_task.done():
            self._paused = True

        deadline = time.time() + max(0.0, float(timeout_s))
        while not self._paused and time.time() < deadline:
        ...
        if (
            ...
            and not self._pause_offloaded
        ):
            self.model.to("cpu")
            self._pause_offloaded = True
            self.torch_platform.empty_cache()
        return True

    async def resume_async_compute(self) -> None:
        if (
            self._pause_offloaded
            and hasattr(self, "model")
            and self.model is not None
        ):
    ...
    async def offload_for_actor_update(self) -> bool:
        """Release reward-side GPU memory before actor backward."""
    ...
    async def _wait_if_pause_requested(self) -> None:
    ...
    async def compute_rewards(self, input_channel: Channel, output_channel: Channel):
        ...
        while True:
            ...
            reward_inputs = [reward_input]
            last_run_counts = [last_run_count]
            await self._drain_reward_inputs_for_merge(
                input_channel,
                reward_inputs,
                last_run_counts,
                mode="train",
            ...
            rewards_list = self._compute_reward_inputs(reward_inputs)
            compute_done = time.perf_counter()

            for rewards in rewards_list:
                rewards = self._normalize_reward_tensor(rewards)

                self.send_reward_output(output_channel, rewards)
            send_done = time.perf_counter()

            if self.reward_profile_enabled:
                batch_sizes = [
                    self._infer_reward_batch_size(item) for item in reward_inputs
                ]
                self._profile_write_jsonl(
                    {
                        "component": "reward",
                        "event": "compute_rewards",
                        "rank": int(self._rank),
                        "request_count": len(reward_inputs),
                        "batch_size": int(sum(batch_sizes)),
                        "batch_sizes": batch_sizes,
                        "last_run_count": int(sum(last_run_counts)),
                        "recv_s": float(recv_done - loop_start),
                        "drain_s": float(drain_done - recv_done),
                        "compute_s": float(compute_done - drain_done),
                        "send_s": float(send_done - compute_done),
                        "elapsed_s": float(send_done - loop_start),
                        "time": time.time(),
                    }
                )

            total_last_run_count += sum(last_run_counts)
    ...
    async def _drain_reward_inputs_for_merge(
        self,
        input_channel: Channel,
        reward_inputs: list[dict[str, Any]],
        last_run_counts: list[int],
        mode: Literal["train", "eval"] = "train",
    ) -> None:
        waited_s = 0.0
        while len(reward_inputs) < self.reward_merge_max_requests:
            next_reward_input = self.try_recv_merged_reward_input_nowait(
                input_channel, mode=mode
            )
            if next_reward_input is None:
                if waited_s >= self.reward_merge_wait_timeout_s:
                    break
                await asyncio.sleep(self.reward_merge_wait_poll_s)
                waited_s += self.reward_merge_wait_poll_s
                continue

            reward_input_i, last_run_count_i = next_reward_input
            reward_inputs.append(reward_input_i)
            last_run_counts.append(last_run_count_i)

    def _compute_reward_inputs(
        self, reward_inputs: list[dict[str, Any]]
    ) -> list[torch.Tensor | np.ndarray]:
        if (
            len(reward_inputs) > 1
            and hasattr(self.model, "compute_reward_sequence")
            and callable(self.model.compute_reward_sequence)
        ):
            return list(self.model.compute_reward_sequence(reward_inputs))
        return [self.model.compute_reward(reward_input) for reward_input in reward_inputs]
    ...
    def try_recv_merged_reward_input_nowait(
        self, input_channel: Channel, mode: Literal["train", "eval"] = "train"
        ...
        if any(input_channel.qsize(key=key) <= 0 for key in keys):
            return None

        batches: list[dict[str, Any]] = []
        last_run_count = 0
        try:
            for (src_rank, expected_size), key in zip(src_ranks_and_sizes, keys):
                data = input_channel.get_nowait(key=key)
                actual_size = self._infer_reward_batch_size(data)
                assert actual_size == expected_size, (
                    f"Expected reward input batch size {expected_size} from env rank {src_rank}, "
                    f"got batch size {actual_size}."
    ...
    def _merge_reward_input_batches(
        ...
        return cat_list_of_dict_tensor(
    ...
    def _setup_dst_ranks(self, batch_size: int) -> list[tuple[int, int]]:
        ...
        return CommMapper.get_dst_ranks_by_partition(
            batch_size=batch_size,
            src_world_size=reward_world_size,
            dst_world_size=env_world_size,
            src_rank=self._rank,
            src_weights=self.cfg.reward.get("routing_weights", None),
    ...
    def _setup_src_ranks(self, batch_size: int) -> list[tuple[int, int]]:
        ...
        env_world_size = self.placement.get_world_size("env")
        reward_world_size = self.placement.get_world_size("reward")
        return CommMapper.get_src_ranks_by_partition(
            batch_size=batch_size,
            src_world_size=env_world_size,
            dst_world_size=reward_world_size,
            dst_rank=self._rank,
            dst_weights=self.cfg.reward.get("routing_weights", None),
            src_align_to=self.cfg.env.train.group_size,
            dst_align_to=self.cfg.env.train.group_size,
        )
    ...
    def send_reward_output(
        ...
        """
        reward_tensor = self._normalize_reward_tensor(reward_tensor)
    ...
    async def _compute_rewards(self, input_channel: Channel, output_channel: Channel):
        while True:
            await self._wait_if_pause_requested()
            reward_input, last_run_count = await self.recv_merged_reward_input(
                input_channel, mode="train"
            )
            reward_inputs = [reward_input]
            ...
            for rewards in self._compute_reward_inputs(reward_inputs):
                self.send_reward_output(output_channel, rewards)
