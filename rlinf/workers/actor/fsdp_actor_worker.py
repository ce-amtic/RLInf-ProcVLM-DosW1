...
def _sanitize_nested_float_tensors(value):
    if isinstance(value, torch.Tensor):
        return _sanitize_float_tensor(value)
    if isinstance(value, dict):
        return {key: _sanitize_nested_float_tensors(val) for key, val in value.items()}
    return value
...
class EmbodiedFSDPActor(FSDPModelManager, Worker):
    def __init__(self, cfg: DictConfig):
        ...
        self.offload_optimizer_with_param = self.cfg.actor.get(
            "offload_optimizer", self.enable_offload
        )
    ...
    def init_worker(self) -> None:
        ...
        if self.enable_offload:
            self.offload_param_and_grad()
            if self.offload_optimizer_with_param:
                self.offload_optimizer()
    ...
    async def sync_model_to_rollout(self) -> None:
        if self.enable_offload:
            if (
                self.offload_optimizer_with_param
                and not self.is_optimizer_offloaded
            ):
                self.offload_optimizer()
    ...
    def _process_received_rollout_batch(
        ...
        if isinstance(forward_inputs, dict):
            valid_action_mask = forward_inputs.pop("valid_action_mask", None)
        if valid_action_mask is not None:
            valid_action_mask = valid_action_mask.to(torch.bool)
            if (
                self.cfg.algorithm.reward_type == "chunk_level"
                and valid_action_mask.dim() >= 3
            ):
                valid_action_mask = valid_action_mask.all(dim=-1, keepdim=True)
            if rollout_batch.get("loss_mask", None) is not None:
                rollout_batch["loss_mask"] = rollout_batch["loss_mask"] & valid_action_mask
            else:
                rollout_batch["loss_mask"] = valid_action_mask
            rollout_batch["loss_mask_sum"] = (
                rollout_batch["loss_mask"].sum(dim=(0, 2), keepdim=True).expand_as(
    ...
    def compute_advantages_and_returns(self) -> dict[str, torch.Tensor]:
        ...
        Compute the advantages and returns.
        """
        self.rollout_batch["rewards"] = _sanitize_float_tensor(
            self.rollout_batch.get("rewards"), nan=0.0, posinf=1e4, neginf=-1e4
        )
        self.rollout_batch["prev_values"] = _sanitize_float_tensor(
            self.rollout_batch.get("prev_values"),
            nan=0.0,
            posinf=1e4,
            neginf=-1e4,
            clamp=(-1e4, 1e4),
        ...
        advantages_and_returns["advantages"] = _sanitize_float_tensor(
            advantages_and_returns.get("advantages"),
            nan=0.0,
            posinf=0.0,
            neginf=0.0,
        )
        advantages_and_returns["returns"] = _sanitize_float_tensor(
            advantages_and_returns.get("returns"),
            nan=0.0,
            posinf=1e4,
            neginf=-1e4,
            clamp=(-1e4, 1e4),
    ...
    def run_training(self) -> None:
        ...
        log_every_n_global_batches = int(
            self.cfg.actor.get("log_every_n_global_batches", 0) or 0
        )
        update_start_time = time.perf_counter()
        if self._rank == 0:
            action_logprob_shape = tuple(
                self.rollout_batch["prev_logprobs"].shape[1:]
            )
            self.logger.info(
                "Actor update start: "
                ...
                f"action_logprob_shape={action_logprob_shape}"
            )
        for epoch_idx in range(update_epoch):
            ...
            for global_batch_idx, train_global_batch in enumerate(
                ...
                global_batch_start_time = time.perf_counter()
                ...
                for idx, batch in enumerate(train_micro_batch):
                    ...
                    if isinstance(forward_inputs, dict):
                        ...
                        if valid_action_mask is not None:
                            valid_action_mask = valid_action_mask.to(torch.bool)
                            if self.cfg.algorithm.reward_type == "chunk_level":
                                valid_action_mask = valid_action_mask.all(
                                    dim=-1, keepdim=True
                                )
                            if loss_mask is None:
                                loss_mask = valid_action_mask
                            else:
                                loss_mask = loss_mask & valid_action_mask
                        forward_inputs = _sanitize_nested_float_tensors(forward_inputs)

                    advantages = _sanitize_float_tensor(
                        advantages, nan=0.0, posinf=0.0, neginf=0.0
                    )
                    prev_logprobs = _sanitize_float_tensor(
                        prev_logprobs,
                        nan=0.0,
                        posinf=100.0,
                        neginf=-100.0,
                    ...
                    returns = _sanitize_float_tensor(
                        returns,
                        nan=0.0,
                        posinf=1e4,
                        neginf=-1e4,
                    ...
                    prev_values = _sanitize_float_tensor(
                        prev_values,
                        nan=0.0,
                        posinf=1e4,
                        neginf=-1e4,
                    ...
                    output_finite_mask = None
                    for output_key in ("logprobs", "values"):
                        output_value = output_dict.get(output_key)
                        if output_value is None or not output_value.dtype.is_floating_point:
                            continue
                        finite_mask = torch.isfinite(output_value).reshape(
                            output_value.shape[0], -1
                        ).all(dim=-1, keepdim=True)
                        output_finite_mask = (
                            finite_mask
                            if output_finite_mask is None
                            else output_finite_mask & finite_mask
                        )
                    if output_finite_mask is not None:
                        if loss_mask is None:
                            loss_mask = output_finite_mask
                        else:
                            while output_finite_mask.dim() < loss_mask.dim():
                                output_finite_mask = output_finite_mask.unsqueeze(-1)
                            loss_mask = loss_mask & output_finite_mask
                    output_dict["logprobs"] = _sanitize_float_tensor(
                        output_dict.get("logprobs"),
                        nan=0.0,
                        posinf=100.0,
                        neginf=-100.0,
                        clamp=(-100.0, 100.0),
                    )
                    output_dict["values"] = _sanitize_float_tensor(
                        output_dict.get("values"),
                        nan=0.0,
                        posinf=1e4,
                        neginf=-1e4,
                        clamp=(-1e4, 1e4),
                    )
                    output_dict["entropy"] = _sanitize_float_tensor(
                        output_dict.get("entropy"), nan=0.0, posinf=0.0, neginf=0.0
                    )
                ...
                if self._rank == 0 and log_every_n_global_batches > 0:
                    should_log_progress = (
                        global_batch_idx == 0
                        or (global_batch_idx + 1) % log_every_n_global_batches == 0
                        or (global_batch_idx + 1) == num_global_batches
                    )
                    if should_log_progress:
                        ...
                        self.logger.info(
                            ...
                            f"global_batch={global_batch_idx + 1}/"
                            f"{num_global_batches}, "
                            f"elapsed_s={now - update_start_time:.1f}, "
                            f"last_global_batch_s="
                            f"{now - global_batch_start_time:.1f}, "
                            f"optimizer_step={self.optimizer_steps}"
                        )
