...
class AsyncPPOEmbodiedRunner(EmbodiedRunner):
    ...
    def __init__(
        ...
        self.pause_reward_during_actor_training = bool(
            self.cfg.runner.get("pause_reward_during_actor_training", False)
        )
        self.reward_pause_timeout_s = float(
            self.cfg.runner.get("reward_pause_timeout_s", 2.0)
        )
        self.reward_offload_during_actor_training = bool(
            self.cfg.reward.get("offload_during_actor_training", True)
        )
    ...
    def run(self) -> None:
        ...
        if self.reward is not None:
            self.reward.set_global_step(self.global_step).wait()
        ...
        while self.global_step < self.max_steps:
            with self.timer("step"):
                ...
                if self.pause_reward_during_actor_training and self.reward is not None:
                    self.reward.pause_async_compute(
                        offload_model=self.reward_offload_during_actor_training,
                        timeout_s=self.reward_pause_timeout_s,
                    ).wait()
                ...
                try:
                    ...
                    if (
                        self.pause_reward_during_actor_training
                        and self.reward is not None
                    ):
                        self.reward.resume_async_compute().wait()
