...
class EnvOutput:
    ...
    def prepare_observations(self, obs: dict[str, Any]) -> dict[str, Any]:
        ...
        reward_image_tensor = (
            obs["reward_main_images"] if "reward_main_images" in obs else None
        )
        ...
        return {
            ...
            "reward_main_images": reward_image_tensor,  # [N_ENV, T, H, W, C]
            "states": states,
...
class EmbodiedRolloutResult:
    ...
    @staticmethod
    def _infer_batch_size_from_value(value: Any) -> int | None:
        if isinstance(value, torch.Tensor):
            if value.dim() == 0:
                raise ValueError("Cannot split a scalar tensor by batch dimension.")
            return int(value.shape[0])
        if isinstance(value, dict):
            for nested_value in value.values():
                batch_size = EmbodiedRolloutResult._infer_batch_size_from_value(
                    nested_value
                )
    ...
    def _infer_batch_size(self) -> int | None:
        ...
        for values in field_lists:
            if values:
                batch_size = self._infer_batch_size_from_value(values[0])
                if batch_size is not None:
                    return batch_size
        return None

    @staticmethod
    def _split_ranges(batch_size: int, split_size: int) -> list[tuple[int, int]]:
        chunks = torch.empty(batch_size).chunk(split_size, dim=0)
        if len(chunks) != split_size:
            raise ValueError(
                f"Cannot split batch size {batch_size} into {split_size} non-empty chunks."
            )
    ...
    @staticmethod
    def _stack_tensor_batch_slice(
        values: list[torch.Tensor], start: int, length: int
    ) -> torch.Tensor:
        return (
            torch.stack(
                [value.narrow(0, start, length) for value in values],
                dim=0,
            )
            .cpu()
            .contiguous()
    ...
    @classmethod
    def _stack_dict_batch_slice(
        cls, values: list[dict[str, Any]], start: int, length: int
    ) -> dict[str, Any]:
        if not values:
            return {}
        ...
        for key in values[0].keys():
            value = values[0][key]
            if isinstance(value, torch.Tensor):
                stacked[key] = cls._stack_tensor_batch_slice(
                    [item[key] for item in values], start, length
                )
            elif isinstance(value, dict):
                stacked[key] = cls._stack_dict_batch_slice(
                    [item[key] for item in values], start, length
                )
            elif value is None:
                continue
    ...
    def iter_splited_trajectories(self, split_size: int):
        batch_size = self._infer_batch_size()
        if batch_size is None:
            model_weights_id = get_model_weights_id(torch.zeros(1, dtype=torch.float32))
        ...
        model_weights_id = (
            get_model_weights_id(torch.stack(self.versions, dim=0).cpu().contiguous())
            if self.versions
            else get_model_weights_id(torch.zeros(1, dtype=torch.float32))
        )

        for start, length in self._split_ranges(batch_size, split_size):
            trajectory = Trajectory(max_episode_length=self.max_episode_length)

            if self.actions:
                trajectory.actions = self._stack_tensor_batch_slice(
                    self.actions, start, length
                )
            if self.intervene_flags:
                trajectory.intervene_flags = self._stack_tensor_batch_slice(
                    self.intervene_flags, start, length
                )
            if self.rewards:
                trajectory.rewards = self._stack_tensor_batch_slice(
                    self.rewards, start, length
                )
            if self.terminations:
                trajectory.terminations = self._stack_tensor_batch_slice(
                    self.terminations, start, length
                )
            if self.truncations:
                trajectory.truncations = self._stack_tensor_batch_slice(
                    self.truncations, start, length
                )
            if self.dones:
                trajectory.dones = self._stack_tensor_batch_slice(
                    self.dones, start, length
                )
            if self.prev_logprobs:
                trajectory.prev_logprobs = self._stack_tensor_batch_slice(
                    self.prev_logprobs, start, length
                )
            if self.prev_values:
                trajectory.prev_values = self._stack_tensor_batch_slice(
                    self.prev_values, start, length
                )
            if self.versions:
                trajectory.versions = self._stack_tensor_batch_slice(
                    self.versions, start, length
                )
            if self.forward_inputs:
                trajectory.forward_inputs = self._stack_dict_batch_slice(
                    self.forward_inputs, start, length
                )
            if self.curr_obs:
                trajectory.curr_obs = self._stack_dict_batch_slice(
                    self.curr_obs, start, length
                )
            if self.next_obs:
                trajectory.next_obs = self._stack_dict_batch_slice(
                    self.next_obs, start, length
                )
    ...
    def to_splited_trajectories(self, split_size: int) -> list[Trajectory]:
        return list(self.iter_splited_trajectories(split_size))
