...
def _validate_eval_env_counts(
    cfg: DictConfig, component_placement: HybridComponentPlacement
) -> None:
    ...
    with open_dict(cfg.env.eval):
        cfg.env.eval.num_parallel_envs = cfg.env.eval.get(
            "num_parallel_envs", cfg.env.eval.total_num_envs
        )

    total_num_envs = int(cfg.env.eval.total_num_envs)
    num_parallel_envs = int(cfg.env.eval.num_parallel_envs)

    assert total_num_envs > 0, (
        "Total number of evaluation environments must be greater than 0"
    )
    assert num_parallel_envs > 0, (
        "Number of parallel evaluation environments must be greater than 0"
    )
    assert total_num_envs % num_parallel_envs == 0, (
        "env.eval.total_num_envs must be divisible by env.eval.num_parallel_envs "
        "for batched evaluation"
    )
    assert num_parallel_envs <= total_num_envs, (
        "env.eval.num_parallel_envs must be less than or equal to "
        "env.eval.total_num_envs"
    )
    assert num_parallel_envs % stage_num == 0, (
        "env.eval.num_parallel_envs must be divisible by rollout.pipeline_stage_num"
    )

    batch_size_per_stage = num_parallel_envs // stage_num
    assert batch_size_per_stage % env_world_size == 0, (
        "env.eval.num_parallel_envs // rollout.pipeline_stage_num must be "
        "divisible by the number of environment processes"
    )
    assert batch_size_per_stage % rollout_world_size == 0, (
        "env.eval.num_parallel_envs // rollout.pipeline_stage_num must be "
        "divisible by the number of rollout processes"
    )
    assert batch_size_per_stage // env_world_size > 0, (
        "env.eval.num_parallel_envs // rollout.pipeline_stage_num // "
        "env_world_size must be greater than 0"
    )
    assert batch_size_per_stage // env_world_size % cfg.env.eval.group_size == 0, (
        "env.eval.num_parallel_envs // rollout.pipeline_stage_num // "
        "env_world_size must be divisible by the group size"
    )
    action_exec_horizon = _resolve_embodied_action_exec_horizon(
        cfg.env.eval, cfg.actor.model.num_action_chunks
    )
    assert (
        cfg.env.eval.max_steps_per_rollout_epoch % action_exec_horizon
        == 0
    ), (
        "env.eval.max_steps_per_rollout_epoch must be divisible by "
        "the env action execution horizon"
    )


def _resolve_embodied_action_exec_horizon(
...
def validate_embodied_cfg(cfg):
    ...
    if cfg.runner.val_check_interval > 0 or cfg.runner.only_eval:
        _validate_eval_env_counts(cfg, component_placement)
    ...
    if not cfg.runner.only_eval:
        ...
        assert (
            ...
            % _resolve_embodied_action_exec_horizon(
                cfg.env.train, cfg.actor.model.num_action_chunks
            )
            == 0
        ), (
            "env.train.max_steps_per_rollout_epoch must be divisible by the env action execution horizon"
        )
