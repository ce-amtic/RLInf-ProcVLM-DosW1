...
def safe_normalize(array, loss_mask):
    if loss_mask is None:
        valid_array = array.reshape(-1)
    else:
        valid_array = array[loss_mask]

    valid_array = valid_array[torch.isfinite(valid_array)]
    if valid_array.numel() > 0:
        mean = valid_array.mean()
        std = valid_array.std(unbiased=False)
        array = (array - mean) / (std + 1e-5)

    return torch.nan_to_num(array, nan=0.0, posinf=0.0, neginf=0.0)
