...
def _finite_training_mask(
    *tensors: Optional[torch.Tensor],
    mask: Optional[torch.Tensor] = None,
) -> torch.Tensor | None:
    def _merge_mask(lhs: torch.Tensor, rhs: torch.Tensor) -> torch.Tensor:
        target_shape = torch.broadcast_shapes(lhs.shape, rhs.shape)
        return torch.broadcast_to(lhs, target_shape) & torch.broadcast_to(
            rhs, target_shape
        )

    finite_mask = mask.bool() if mask is not None else None
    for tensor in tensors:
        if tensor is None:
            continue
        tensor_finite = torch.isfinite(tensor)
        if finite_mask is None:
            finite_mask = tensor_finite
        else:
            finite_mask = _merge_mask(finite_mask, tensor_finite)
    return finite_mask
...
def compute_decoupled_ppo_actor_loss(
    ...
    **kwargs,
    ...
    loss_mask = _finite_training_mask(logprobs, old_logprobs, advantages, mask=loss_mask)
    logprobs = torch.nan_to_num(logprobs, nan=0.0, posinf=100.0, neginf=-100.0)
    old_logprobs = torch.nan_to_num(
        old_logprobs, nan=0.0, posinf=100.0, neginf=-100.0
    )
    ...
    loss_mask = _finite_training_mask(proximal_logprobs, mask=loss_mask)
    proximal_logprobs = torch.nan_to_num(
        proximal_logprobs, nan=0.0, posinf=100.0, neginf=-100.0
    )

    loss_mask_count = loss_mask.count_nonzero() or 1
    proximal_log_ratio = torch.clamp(logprobs - proximal_logprobs, min=-20.0, max=20.0)
    proximal_ratio = torch.where(
        loss_mask, torch.exp(proximal_log_ratio), 0.0
    )
    ...
    behav_log_ratio = torch.clamp(proximal_logprobs - old_logprobs, min=-20.0, max=20.0)
    ...
    with torch.no_grad():
        ...
        proximal_approx_kl = (
            -torch.where(loss_mask, proximal_log_ratio, 0.0).sum()
            / loss_mask_count
        )
        behav_approx_kl = (
            -torch.where(behav_mask, behav_log_ratio, 0.0).sum()
            / behav_mask_count
        )
...
def compute_ppo_critic_loss(
    ...
    **kwargs,
    ...
    loss_mask = _finite_training_mask(values, returns, prev_values, mask=loss_mask)
    ...
    prev_values = torch.nan_to_num(prev_values, nan=0.0, posinf=1e4, neginf=-1e4)
    ...
    if masked_returns.numel() == 0 or torch.isnan(var_returns) or var_returns == 0:
