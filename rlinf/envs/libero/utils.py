...
def patch_libero_init_state_loader() -> None:
    """Make LIBERO init-state loading compatible with PyTorch 2.6+.

    LIBERO init files store numpy arrays. Since PyTorch 2.6, ``torch.load``
    defaults to ``weights_only=True``, which rejects those legacy pickle payloads.
    These files are local benchmark assets, so RLinf loads them with
    ``weights_only=False`` explicitly.
    """

    if getattr(Benchmark, "_rlinf_weights_only_patch", False):
        return
    ...
    Benchmark._rlinf_weights_only_patch = True


patch_libero_init_state_loader()
