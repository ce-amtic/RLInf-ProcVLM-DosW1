...
def _get_tensorflow():
    # NOTE: TensorFlow must NOT be imported in the env-worker process. It coexists there
    # with torch + SAPIEN (vulkan/EGL) and deterministically SIGSEGVs in the first image
    # op (tf.image.convert_image_dtype) due to CUDA library/context conflicts. The image
    # crop below was reimplemented in pure torch (see center_crop_image); this helper is
    # kept only for any out-of-tree caller and is no longer used by the env path.
    global _tf
    if _tf is None:
        try:
...
def center_crop_image(image):
    """Center-crop to ``crop_scale`` * area, then bilinear-resize to 224x224.

    Pure-torch reimplementation of the original ``tf.image.crop_and_resize`` path.
    TensorFlow cannot be used here: in the env worker it coexists with torch +
    SAPIEN (vulkan/EGL) and deterministically SIGSEGVs in tf.image.convert_image_dtype
    (CUDA library/context conflict). The grid below reproduces tf.image.crop_and_resize's
    align_corners bilinear sampling; output matches TF to within +-1 on uint8 (validated).
    """
    crop_scale = 0.9
    ...
    if t.ndim == 2:
        t = t.unsqueeze(-1)
    t = t.div(255.0).permute(2, 0, 1).unsqueeze(0)  # [1, C, H, W], values in [0, 1]

    # Centered box covering sqrt(crop_scale) of each dim; sampled to a 224x224 grid with
    # the same corner-aligned mapping tf.image.crop_and_resize uses.
    s = float(crop_scale) ** 0.5
    y1 = (1.0 - s) / 2.0
    idx = torch.arange(224, dtype=torch.float64)
    coord = y1 + idx * s / 223.0  # normalized [0,1] positions, in [y1, y1 + s]
    g = (2.0 * coord - 1.0).to(torch.float32)  # -> grid_sample's [-1, 1]
    gy = g.view(224, 1).expand(224, 224)
    gx = g.view(1, 224).expand(224, 224)
    grid = torch.stack([gx, gy], dim=-1).unsqueeze(0)  # [1, 224, 224, (x, y)]

    out = torch.nn.functional.grid_sample(
        t, grid, mode="bilinear", align_corners=True, padding_mode="border"
    ).clamp(0.0, 1.0)
    out = out[0].permute(1, 2, 0)  # [224, 224, C]
    u8 = torch.clamp(torch.floor(out * 255.0 + 0.5), 0, 255).to(torch.uint8).numpy()
