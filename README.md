...
Model checkpoints, Python environments, demonstrations, datasets, caches, and
run logs are not versioned. Set `PROCVLM_PYTHON` to a ProcVLM-compatible Python
executable and `PROCVLM_REWARD_MODEL_PATH` to the deployed merged checkpoint.
The checkpoint used by the validated 127 deployment is currently:
