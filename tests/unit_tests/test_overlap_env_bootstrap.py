...
class TestOverlapEnvBootstrap(unittest.TestCase):
    def setUp(self):
        ...
        self.worker.offload_train_env_before_actor_handoff = False
        self.worker.collect_transitions = False
        self.worker.collect_prev_infos = True
        self.worker.use_external_reward_model = False
    ...
    def test_history_reward_skips_empty_interval_tick(self):
        """History reward should not send empty RM requests between interval ticks."""
        self.worker.reward_mode = "history_buffer"
        self.worker.env_infos_reward_keys = []
        self.worker.history_lengths = [{}]
        self.worker.train_history_managers = [
            HistoryManager(
                OmegaConf.create(
                    {
                        "model": {
        ...
        env_output = EnvOutput(
            obs={"main_images": torch.zeros(2, 1)},
            rewards=torch.zeros(2, 4),
            dones=torch.zeros(2, dtype=torch.bool),
        )

        reward_input = self.worker._build_reward_model_input(
            env_output=env_output,
            stage_id=0,
        )

        self.assertIsNone(reward_input)
        self.assertEqual(self.worker.history_lengths[0], {})
